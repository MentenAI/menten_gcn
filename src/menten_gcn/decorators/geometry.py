...
class CACA_dist( Decorator ):
    ...
    def describe_edge_features( self ):
        if self.use_nm:
            ...
            d_units = "Angstroms"        
        return [
            "Euclidean distance between the two CA atoms of each residue, measured in " + d_units,
            ]
...
class CBCB_dist( Decorator ):
    ...
    def describe_edge_features( self ):
        if self.use_nm:
            ...
            d_units = "Angstroms"        
        return [
            "Euclidean distance between the two CB atoms of each residue, measured in " + d_units + ". In the case of GLY, use an estimate of ALA's CB position",
            ]
...
class ChiAngleDecorator( Decorator ):
    ...
    def describe_node_features( self ):
        ...
        for chi in self.chis:
            if self.sincos:
                desc.append( "Sine of chi angle number " + str(chi) + " of each residue. Spans from -1 to 1, 0 if chi angle is not valid for this residue" )
                ...
                desc.append( "Chi angle number " + str(chi) + " of each residue measured in radians. Spans from -pi to pi, -5 if chi angle is not valid for this residue" )
        return desc
